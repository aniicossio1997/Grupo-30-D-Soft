...
//----------------------------------------------------------------------
 ...
 if (isset($_POST['tipo']) && !empty($_POST['tipo']) && $_POST['tipo']==0  && isset($_POST['origen']) && !empty($_POST['origen']) && isset($_POST['destino']) && !empty($_POST['destino']) && isset($_POST['costo']) && !empty($_POST['costo']) && isset($_POST['duracion']) && !empty($_POST['duracion']) && isset($_POST['vehiculo']) && !empty($_POST['vehiculo']) && isset($_POST['horario']) && !empty($_POST['horario']) && isset($_POST['copilotos']) && !empty($_POST['copilotos']) )
 {
	...
	if ($mostrar['asientos']<=$_POST['copilotos']) {
		$_SESSION['mensaje']= "recuerde  que la cantidad de acompañantes debe de ser menor que la cantidad de asientos del vehiculo seleccionado <br>";
		header("Location:agregar_viaje.php");
		die("ERROR acompañantes");

						
	}
	//-----------------------

	$sql="INSERT INTO viajes (id, vehiculo_id, tipo, costo, copilotos, horario, activo,descripcion, fecha)VALUES (NULL, $_POST[vehiculo],'$_POST[tipo]',$_POST[costo],$_POST[copilotos],'$horario',1";
