function limpiar_errores() {
	...
	document.getElementById('marca').classList.add('focus_azul');
	document.getElementById('patente').classList.add('focus_azul');
	document.getElementById('modelo').classList.add('focus_azul');
	document.getElementById('asientos').classList.add('focus_azul');
...
function validar_patente(){
	...
	/*if (!exp_pat.test(patente.value)) {
		document.getElementById('error_patente').innerHTML="El formato de la patente no cumple con lo permitido";
		patente.classList.remove('focus_azul');
		patente.classList.add('error_border');
		patente.focus();
		alert("ERROR: la patente no es valida");
		return false;
	}*/
	return true;
}
...
function validar_cantAsientos(){
	...
	if (!cantAsientos.value){
 	...
 	if (!/^([1-9])*$/.test(cantAsientos.value)){
